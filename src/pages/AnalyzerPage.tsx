...
export const AnalyzerPage: React.FC = () => {
  ...
  const { language, code, activeFilename, setLanguage, setCode, setActiveFilename, setValidationError, setInfoNotification, loadSampleCode, clearCode, recordSubmission } = useCodeContext();
  ...
  const handleFileLoaded = (content: string, fileLanguage: SupportedLanguage, filename: string) => {
    ...
    setInfoNotification(isGenericFile ? `Successfully loaded "${filename}" using ${SUPPORTED_LANGUAGES[fileLanguage].name} for analysis.` : `Successfully loaded "${filename}" (${SUPPORTED_LANGUAGES[fileLanguage].name}).`);
  };
  ...
  const handleAnalyzeClick = async () => {
    ...
    setValidationError(null);
    try {
      const result = await analyzerService.submitForAnalysis({ code, language, filename: activeFilename || undefined, options: { includeAst: true, includeOptimizations: true, includeCodeSmells: true } });
  ...
  return (
    <div className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 flex flex-col gap-6">
      <div className="pb-4 border-b border-zinc-200 dark:border-zinc-800"><h1 className="text-xl sm:text-2xl font-bold tracking-tight text-zinc-900 dark:text-zinc-100">Analyzer Workspace</h1><p className="text-xs sm:text-sm text-zinc-500 dark:text-zinc-400 mt-1">Analyze Java, C, C++, and Python source and view its actual program output.</p></div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 rounded-lg bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-xs"><div className="flex items-center gap-2 text-xs text-zinc-500 dark:text-zinc-400">{isAnalyzing ? <Info className="w-4 h-4 text-emerald-500 shrink-0" /> : <AlertCircle className="w-4 h-4 text-zinc-400 shrink-0" />}<span>{isAnalyzing ? 'Analyzing and running source…' : 'Analysis includes static inspection and a time-limited program run.'}</span></div><div className="flex items-center gap-3 w-full sm:w-auto justify-end"><button type="button" id="clear-code-btn" onClick={clearCode} disabled={isAnalyzing} className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg border border-zinc-300 dark:border-zinc-700 text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-800 text-sm font-medium disabled:opacity-50"><Trash2 className="w-4 h-4" /><span>Clear</span></button><button type="button" id="analyze-code-btn" onClick={handleAnalyzeClick} disabled={isAnalyzing} className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 py-2.5 rounded-lg bg-zinc-900 hover:bg-zinc-800 dark:bg-emerald-600 dark:hover:bg-emerald-500 text-white text-sm font-semibold disabled:opacity-60 disabled:cursor-wait"><Play className="w-4 h-4 fill-current" /><span>{isAnalyzing ? 'Analyzing…' : 'Analyze Code'}</span></button></div></div>
    </div>
  );
};
