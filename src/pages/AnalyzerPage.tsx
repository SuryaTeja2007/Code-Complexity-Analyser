...
export const AnalyzerPage: React.FC = () => {
  ...
  return (
    <div className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-start"><div className="lg:col-span-6"><FileUpload onFileLoaded={handleFileLoaded} onError={(msg) => setValidationError(msg)} activeFilename={activeFilename} /></div><div className="lg:col-span-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3 sm:p-4 rounded-lg bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800"><LanguageSelector currentLanguage={language} onSelectLanguage={handleLanguageChange} /><button type="button" id="load-sample-btn" onClick={() => loadSampleCode(language)} className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-xs font-medium text-zinc-700 dark:text-zinc-300 bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 border border-zinc-200 dark:border-zinc-700"><RotateCcw className="w-3.5 h-3.5" /><span>Load Sample</span></button></div></div>
