...
export const AnalyzerPage: React.FC = () => {
  ...
  const handleFileLoaded = (content: string, detectedLang: SupportedLanguage, filename: string) => {
    setCode(content);
    setLanguage(detectedLang);
    setActiveFilename(filename);
    setValidationError(null);
    setInfoNotification(`Successfully loaded "${filename}" (${SUPPORTED_LANGUAGES[detectedLang].name}).`);
  };
  ...
  return (
    <div className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 flex flex-col gap-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-zinc-200 dark:border-zinc-800">
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800/60 text-xs text-emerald-700 dark:text-emerald-300 shrink-0">
          <Info className="w-4 h-4 shrink-0" />
          <span>Phase 2: Backend API Foundation</span>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-start">
        <div className="lg:col-span-6">
          <FileUpload onFileLoaded={handleFileLoaded} onError={(msg) => setValidationError(msg)} activeFilename={activeFilename} />
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 px-4 py-2.5 bg-zinc-100 dark:bg-zinc-900 rounded-t-lg border-t border-x border-zinc-200 dark:border-zinc-800 text-xs">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1.5 font-medium text-zinc-700 dark:text-zinc-300">
              ...
              <span>{activeFilename ? activeFilename : `Editor (${SUPPORTED_LANGUAGES[language].name})`}</span>
            </div>
            <span className="text-zinc-300 dark:text-zinc-700">|</span>
            <span className="text-zinc-500 dark:text-zinc-400 text-[11px]">Editable snippet, method, or program</span>
          </div>
          <div className="flex items-center gap-4 text-zinc-500 dark:text-zinc-400 font-mono text-[11px]">
            <span>{lineCount} {lineCount === 1 ? 'line' : 'lines'}</span><span>•</span><span>{charCount} chars</span>
          </div>
        </div>
        <CodeEditor code={code} language={language} onChange={(newVal) => setCode(newVal || '')} height="520px" />
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 rounded-lg bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-xs">
        <div className="flex items-center gap-2 text-xs text-zinc-500 dark:text-zinc-400">
          {isAnalyzing ? <Info className="w-4 h-4 text-emerald-500 shrink-0" /> : <AlertCircle className="w-4 h-4 text-zinc-400 shrink-0" />}
          <span>{isAnalyzing ? 'Sending source code to the analysis backend…' : 'The backend validates the request without executing user code.'}</span>
        </div>
        <div className="flex items-center gap-3 w-full sm:w-auto justify-end">
          <button type="button" id="clear-code-btn" onClick={clearCode} disabled={isAnalyzing} className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg border border-zinc-300 dark:border-zinc-700 text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-800 text-sm font-medium disabled:opacity-50">
            <Trash2 className="w-4 h-4 text-zinc-500 dark:text-zinc-400" /><span>Clear</span>
          </button>
          <button type="button" id="analyze-code-btn" onClick={handleAnalyzeClick} disabled={isAnalyzing} className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 py-2.5 rounded-lg bg-zinc-900 hover:bg-zinc-800 dark:bg-emerald-600 dark:hover:bg-emerald-500 text-white text-sm font-semibold transition-colors shadow-sm disabled:opacity-60 disabled:cursor-wait focus:outline-none focus:ring-2 focus:ring-emerald-500">
            <Play className="w-4 h-4 fill-current" /><span>{isAnalyzing ? 'Connecting…' : 'Analyze Code'}</span>
          </button>
        </div>
      </div>
    </div>
